signo(aplausos, frases_comunes, 'FRA001').
signo(buen_provecho, frases_comunes, 'FRA002').
signo(como_estas, frases_comunes, 'FRA003').
signo(con_permiso, frases_comunes, 'FRA004').
signo(de_nada, frases_comunes, 'FRA005').
signo(dios_te_bendiga, frases_comunes, 'FRA006').
signo(disculpa, frases_comunes, 'FRA007').
signo(entendiste, frases_comunes, 'FRA008').
signo(feliz_cumpleanos, frases_comunes, 'FRA009').
signo(frases_cortesia, frases_comunes, 'FRA010').
signo(gracias, frases_comunes, 'FRA011').
signo(me_equivoque, frases_comunes, 'FRA012').
signo(me_gusta, frases_comunes, 'FRA013').
signo(mi_apodo_es, frases_comunes, 'FRA014').
signo(mi_nombre, frases_comunes, 'FRA015').
signo(mucho_gusto, frases_comunes, 'FRA016').
signo(no, frases_comunes, 'FRA017').
signo(no_importa, frases_comunes, 'FRA018').
signo(no_se, frases_comunes, 'FRA019').
signo(ok, frases_comunes, 'FRA020').
signo(permiso, frases_comunes, 'FRA021').
signo(porfavor, frases_comunes, 'FRA022').
signo(si, frases_comunes, 'FRA023').
signo(tal_vez, frases_comunes, 'FRA024').

video_youtube('FRA001', 'https://youtu.be/ZsqZj7Y9rG0'). % aplausos
video_youtube('FRA002', 'https://youtu.be/BKkcyJK052o'). % buen_provecho
video_youtube('FRA003', 'https://youtu.be/H512gQZ9cIM'). % como_estas
video_youtube('FRA004', 'https://youtu.be/jeVu_asR_RM'). % con_permiso
video_youtube('FRA005', 'https://youtu.be/5ygu60J0yQI'). % de_nada
video_youtube('FRA006', 'https://youtu.be/SVEND2sfPIA'). % dios_te_bendiga
video_youtube('FRA007', 'https://youtu.be/VSOGNvHPWkA'). % disculpa
video_youtube('FRA008', 'https://youtu.be/1UixnZTC4Cs'). % entendiste
video_youtube('FRA009', 'https://youtu.be/D_Tvuk9zbAg'). % feliz_cumpleanos
video_youtube('FRA010', 'https://youtu.be/xCz6Tvcl_sw'). % frases_cortesia
video_youtube('FRA011', 'https://youtu.be/AM5CKJdPGCY'). % gracias
video_youtube('FRA012', 'https://youtu.be/Kx2m6ikcYb8'). % me_equivoque
video_youtube('FRA013', 'https://youtu.be/FM3uvW7w4w0'). % me_gusta
video_youtube('FRA014', 'https://youtu.be/BQdGKv4mvDg'). % mi_apodo_es
video_youtube('FRA015', 'https://youtu.be/1k2oJwqZkkM'). % mi_nombre
video_youtube('FRA016', 'https://youtu.be/qYwMfWLCIJM'). % mucho_gusto
video_youtube('FRA017', 'https://youtu.be/02paHBZpUyM'). % no
video_youtube('FRA018', 'https://youtu.be/TnI2pGptYYk'). % no_importa
video_youtube('FRA019', 'https://youtu.be/FHsr92WZ6tg'). % no_se
video_youtube('FRA020', 'https://youtu.be/JPjVZoISjjI'). % ok
video_youtube('FRA021', 'https://youtu.be/jeVu_asR_RM'). % permiso
video_youtube('FRA022', 'https://youtu.be/0usq3kl7F_8'). % porfavor
video_youtube('FRA023', 'https://youtu.be/8ouWLJw3z4E'). % si
video_youtube('FRA024', 'https://youtu.be/LG6Nj6fWqzI'). % tal_vez
